...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
    ...
    def union(that: TweetSet): TweetSet = 
      // that.incl(elem).union(left)
      left.union(right).union(that).incl(elem) //?????

    def mostRetweeted: Tweet = 
      if (left.isEmpty && right.isEmpty) elem
      else if (left.isEmpty) elem.moreRetweeted(right.mostRetweeted)
      else if (right.isEmpty) elem.moreRetweeted(left.mostRetweeted)
      else elem.moreRetweeted(left.mostRetweeted).moreRetweeted(right.mostRetweeted)
